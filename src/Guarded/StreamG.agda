...
headˢ : {A : 𝒰} → Stream A → A
headˢ (cons x xs) = x

tail▹ˢ : {A : 𝒰} → Stream A → ▹ Stream A
tail▹ˢ (cons x xs) = xs

repeatˢ : {A : 𝒰} → A → Stream A
repeatˢ a = fix (cons a)

repeatˢ-eq : {A : 𝒰} (a : A) → repeatˢ a ＝ cons a (λ α → repeatˢ a)
repeatˢ-eq a = ap (cons a) (pfix (cons a))

mapˢ : {A B : 𝒰} → (A → B) → Stream A → Stream B
...
mapˢ-repeat : {A B : 𝒰} → (a : A) → (f : A → B) → mapˢ f (repeatˢ a) ＝ repeatˢ (f a)
...
scanl1ˢ : {A : 𝒰} → (f : A → A → A) → Stream A → Stream A
